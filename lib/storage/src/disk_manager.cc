...
auto disk_manager::open(const std::filesystem::path& path) -> result<stdx::box<disk_manager>> {
    ...
    if (!file.is_open()) {
        // A file not opening might mean creation is necessary
        {
            ...
            if (!create.is_open()) { return stdx::err{error::IO_ERROR}; }
        }
        file.open(path, open_mode);
        if (!file.is_open()) { return stdx::err{error::IO_ERROR}; }
    }
...
auto disk_manager::read_page(page_id_t pid, read_buf_t buf) -> result<void> {
    ...
    if (id < 0 || id >= num_pages_) { return stdx::err{error::STORAGE_INVALID_PAGE_ID}; }
    ...
    TRY(io_utils::run_io(file_, [&] -> result<void> {
        ...
        if (file_.eof()) { return stdx::err{error::STORAGE_SHORT_READ}; }
        return {};
    }));
    return {};
}
