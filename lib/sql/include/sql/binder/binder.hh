...
template <usize PoolSize> class binder_t {
  ...
  public:
    ...
    [[nodiscard]] auto bind(const parser::ast_t& tree, node_id_t root_id)
        ...
        ast_t ast;
        auto  bound_root{TRY(tree[root_id].visit(
            [&](const auto& node) { return (*this)(node, tree, root_id, ast); }))};
        ast.add_root(bound_root);
        return ast;
    }
    ...
    [[nodiscard]] auto bind_expression(const parser::ast_t&                tree,
        ...
        return tree[expr_id].visit(
            [&](const auto& node) { return (*this)(node, tree, expr_id, scopes, ast); });
    }

    [[nodiscard]] auto operator()(stdx::monostate, const parser::ast_t& tree, node_id_t id, ast_t&)
        -> stdx::result<node_id_t, diagnostic> {
        return stdx::err{diagnostic{error::IO_ERROR, tree.get_location(id)}};
    }

    [[nodiscard]] auto operator()(const parser::select_stmt_t& select,
                                  const parser::ast_t&         tree,
                                  node_id_t                    id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        return bind_select(tree, id, select, ast);
    }

    [[nodiscard]] auto operator()(const parser::create_table_stmt_t& create,
                                  const parser::ast_t&               tree,
                                  node_id_t                          id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        const auto loc{tree.get_location(id)};
        auto       tbl_name{create.table_name};
        if (tbl_name.empty()) { return stdx::err{diagnostic{error::SQL_TABLE_NOT_FOUND, loc}}; }
        ...
        return ast.template add_node<create_table_stmt_t>(loc, tbl_name, create.column_defs);
    }

    [[nodiscard]] auto operator()(const parser::drop_table_stmt_t& drop,
                                  const parser::ast_t&             tree,
                                  node_id_t                        id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        const auto loc{tree.get_location(id)};
        ...
        return ast.template add_node<drop_table_stmt_t>(loc, tbl->table_id, tbl_name);
    }

    [[nodiscard]] auto operator()(const parser::alter_table_stmt_t& alter,
                                  const parser::ast_t&              tree,
                                  node_id_t                         id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        const auto loc{tree.get_location(id)};
        ...
        const auto it{std::ranges::find_if(
            columns, [&](const auto& col) { return col.name() == alter.column_def.name.view(); })};
        ...
        return ast.template add_node<alter_table_stmt_t>(
            loc, tbl->table_id, tbl_name, alter.column_def);
    }

    [[nodiscard]] auto operator()(const parser::create_index_stmt_t& idx,
                                  const parser::ast_t&               tree,
                                  node_id_t                          id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        const auto loc{tree.get_location(id)};
        ...
        for (const auto& col_name_fixed : idx.columns) {
            ...
            col_indices.emplace_back(static_cast<usize>(std::distance(columns.begin(), it)));
        }

        return ast.template add_node<create_index_stmt_t>(loc,
                                                          idx.index_name,
                                                          tbl->table_id,
                                                          tbl_name,
                                                          std::move(col_names),
    ...
    [[nodiscard]] auto operator()(const parser::drop_index_stmt_t& drop_idx,
                                  const parser::ast_t&             tree,
                                  node_id_t                        id,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        const auto          loc{tree.get_location(id)};
        ...
        return ast.template add_node<drop_index_stmt_t>(
            loc, drop_idx.index_name, tbl->table_id, tbl_name);
    }

    template <typename T>
    [[nodiscard]] auto operator()(const T&, const parser::ast_t& tree, node_id_t id, ast_t&)
        -> stdx::result<node_id_t, diagnostic> {
        return stdx::err{diagnostic{error::IO_ERROR, tree.get_location(id)}};
    }

    [[nodiscard]] auto operator()(stdx::monostate,
                                  const parser::ast_t& tree,
                                  node_id_t            id,
                                  const std::vector<binding_scope_t>&,
                                  ast_t&) -> stdx::result<node_id_t, diagnostic> {
        return stdx::err{diagnostic{error::IO_ERROR, tree.get_location(id)}};
    }

    [[nodiscard]] auto operator()(const parser::literal_expr_t& lit,
                                  const parser::ast_t&          tree,
                                  node_id_t                     id,
                                  const std::vector<binding_scope_t>&,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
    ...
    [[nodiscard]] auto operator()(const parser::identifier_expr_t&    ident,
                                  const parser::ast_t&                tree,
                                  node_id_t                           id,
                                  const std::vector<binding_scope_t>& scopes,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        ...
        if (!prefix.empty()) {
            const auto scope_it{std::find_if(scopes.begin(), scopes.end(), [&](const auto& s) {
            ...
            const auto col_it{std::find_if(columns.begin(), columns.end(), [&](const auto& col) {
            ...
            return ast.template add_node<column_ref_expr_t>(
                loc, matched_scope->table_id, col_idx, stdx::fixed::string{col_name}, col.type());
        } else {
            ...
            for (const auto& scope : scopes) {
                ...
                const auto it{std::find_if(columns.begin(), columns.end(), [&](const auto& col) {
                    return col.name() == col_name;
                })};
                if (it != columns.end()) {
            ...
            if (match_count > 1) {
                return stdx::err{diagnostic{error::SQL_COLUMN_AMBIGUOUS, loc.line, loc.column}};
            }
            if (match_count == 1) {
    ...
    [[nodiscard]] auto operator()(const parser::aggregate_expr_t&     agg,
                                  const parser::ast_t&                tree,
                                  node_id_t                           id,
                                  const std::vector<binding_scope_t>& scopes,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        ...
        case parser::agg_func_t::MAX:
            if (!arg_type) { return stdx::err{diagnostic{error::SQL_INVALID_AGGREGATE, loc}}; }
            ret_type = arg_type;
            break;
        }
    ...
    [[nodiscard]] auto operator()(const parser::binary_expr_t&        binary,
                                  const parser::ast_t&                tree,
                                  node_id_t                           id,
                                  const std::vector<binding_scope_t>& scopes,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        ...
        case parser::binary_op_t::GTEQ: {
            ...
            if (!target_type_opt) { return stdx::err{diagnostic{error::SQL_TYPE_MISMATCH, loc}}; }
            ...
            if (l_type != target_type) {
                ...
                bound_lhs_id = ast.template add_node<cast_expr_t>(loc, bound_lhs_id, target_type);
            }
            ...
            if (r_type != target_type) {
                ...
                bound_rhs_id = ast.template add_node<cast_expr_t>(loc, bound_rhs_id, target_type);
            }
            res_type.emplace(type::id_t::BOOLEAN);
            break;
        }
    ...
    [[nodiscard]] auto operator()(const parser::unary_expr_t&         unary,
                                  const parser::ast_t&                tree,
                                  node_id_t                           id,
                                  const std::vector<binding_scope_t>& scopes,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
    ...
    [[nodiscard]] auto operator()(const parser::function_expr_t&      func,
                                  const parser::ast_t&                tree,
                                  node_id_t                           id,
                                  const std::vector<binding_scope_t>& scopes,
                                  ast_t& ast) -> stdx::result<node_id_t, diagnostic> {
        ...
        if (string_utils::iequals{}(name_view, "COALESCE")) {
            if (bound_args.empty()) { return stdx::err{diagnostic{error::SQL_TYPE_MISMATCH, loc}}; }
            ...
            for (usize i{0}; i < bound_args.size(); ++i) {
                if (arg_types[i] != common) {
                    ...
                    bound_args[i] = ast.template add_node<cast_expr_t>(loc, bound_args[i], *common);
                }
            }
            ...
            if (arg_types[0] != type::id_t::VARCHAR) {
                ...
                bound_args[0] =
                    ast.template add_node<cast_expr_t>(loc, bound_args[0], type::id_t::VARCHAR);
            }
            ...
            for (usize i{1}; i < 3; ++i) {
                ...
                if (arg_types[i] != type::id_t::INTEGER) {
                    ...
                    bound_args[i] =
                        ast.template add_node<cast_expr_t>(loc, bound_args[i], type::id_t::INTEGER);
                }
            }
